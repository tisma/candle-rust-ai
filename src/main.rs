use candle_core::{Tensor, Module, Device, D, Result};
use candle_nn::{Linear, VarBuilder, VarMap, loss, ops, Optimizer};
...
pub fn main() -> anyhow::Result<()> {
    ...
    let test_results_tensor = Tensor::from_vec(test_results_vec.clone(), test_votes_vec.len() / VOTE_DIM, &dev)?;
    ...
    let tensor_test_votes = Tensor::from_vec(
        ...
        (real_world_votes.len() / VOTE_DIM, VOTE_DIM),
        &dev
    )?
    .to_dtype(candle_core::DType::F32)?;
    ...
    let result = final_result.argmax(D::Minus1)?
    .to_dtype(candle_core::DType::F32)?
    .get(0)
...
fn train(m: Dataset, dev: &Device) -> anyhow::Result<MultiLevelPerceptron> {
    ...
    for epoch in 1..EPOCHS + 1 {
        ...
        println!("Epoch: {epoch:3} Train loss: {:8.5} Test accuracy: {:5.2}%", loss.to_scalar::<f32>()?, final_accuracy);
